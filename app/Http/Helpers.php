...
function convert_price1($price,$rate)
{
    if (Session::has('currency_code') && (Session::get('currency_code') != get_system_default_currency()->code)) {
        ...
        foreach ($rate as $item)
        {
            if ($item->code == "SY")
            {
                $price = floatval($price) / floatval($item->exchange_rate);
                break;
            }
        }
        foreach ($rate as $item)
        {
            if (Session::get('currency_code') == $item->code){
                $price = floatval($price) * floatval(Session::get('currency_exchange_rate'));
                break;
            }
        }
...
function single_price1($price,$rate)
{
    return format_price(convert_price1($price,$rate));
}
